...
bool VoodooPS2TouchPadBase::start( IOService * provider )
{
    ...
    xscrollTimer = IOTimerEventSource::timerEventSource(this, OSMemberFunctionCast(IOTimerEventSource::Action, this, &VoodooPS2TouchPadBase::onScrollTimerX));
    if (xscrollTimer)
...
bool VoodooPS2TouchPadBase::publish_multitouch_interface() {
...
void VoodooPS2TouchPadBase::unpublish_multitouch_interface() {
    if (mt_interface) {
...
void VoodooPS2TouchPadBase::onScrollTimerX(void)
{
    //
...
int VoodooPS2TouchPadBase::synaptics_parse_hw_state(const UInt8 buf[])
{
    if(!mt_interface) {
        return -1;
    }
...
void VoodooPS2TouchPadBase::parse_input(UInt8* packet, UInt32 packetSize) {
